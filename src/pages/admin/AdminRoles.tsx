...
import { Shield, Search, MoreHorizontal, UserPlus, UserMinus, Crown, Users, UserCog, Key } from 'lucide-react';
  ...
  const handleAddRole = async () => {
    ...
    try {
      const { error } = await supabase
        .from('user_roles')
        .insert({ user_id: selectedUser.user_id, role: roleToAdd });
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Manage users, roles, and custom permissions</p>
        </div>
